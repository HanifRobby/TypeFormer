    ...
    def __init__(
        ...
        for i in range(len(self.Dataset)):
            ...
            for j in range(len(self.Dataset[i])):
                if self.db == "Aalto_mobile" and self.Dataset[i][j].shape[1] > 5:
        ...
        if self.db == "Clarkson_II":
            self.Dataset = np.delete(
                self.Dataset, [x for x in range(50, 150)], axis=2
            )  # only for clarkson II
        for i in range(len(self.Dataset)):
            # print(i)
            for j in range(len(self.Dataset[i])):
                self.Dataset[i][j] = np.concatenate(
                    (self.Dataset[i][j], np.zeros((self.data_length, dimension)))
                )[: self.data_length]
...
def extract_test_keystroke_features(session_1, sequence_length=100):
    hold_time_1 = np.reshape(
        (session_1[:, 1] - session_1[:, 0]) / 1e3, (np.shape(session_1)[0], 1)
    ).astype(np.float32)
    inter_press_1 = np.reshape(
        np.append(0, np.diff(session_1[:, 0])) / 1e3, (np.shape(session_1)[0], 1)
    ).astype(np.float32)
    inter_release_1 = np.reshape(
        np.append(0, np.diff(session_1[:, 1])) / 1e3, (np.shape(session_1)[0], 1)
    ).astype(np.float32)
    inter_key_1 = np.reshape(
        np.append(0, session_1[:-1, 1] - session_1[1:, 0]) / 1e3,
        (np.shape(session_1)[0], 1),
    ).astype(np.float32)
    ...
    session_1_processed = np.concatenate(
        (
            session_1_processed,
            np.zeros((sequence_length, np.shape(session_1_processed)[1])),
        )
    )[:sequence_length]
